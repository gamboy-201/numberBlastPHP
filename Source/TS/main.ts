...
namespace surya_1 {
    ...
    export class Main {
        ...
        private firstDrop: any;
        private tweenedDownPlayer: any[];
        private pathArrCircle: any;
        constructor() {
        ...
        private drawBlockShapes(i: number): void {
            ...
            for (let j = 0; j < 8; j++) {
                ...
                if (randomValue == 0) {
                    ...
                    this.destructibleCubeArray[i][this.blockCount] = new CollisionShape(true, "rect", j * 50, i * 50, 50, 50, { isStatic: true, friction: 0, restitution: 1, collisionFilter: { category: 0x0001 } });
                    ...
                    if (randomValue === 1) {
                        ...
                        newSprite.beginFill(0xffffff, 1).drawCircle(0, 0, circleRad).endFill();
                        ...
                        let outerCircle = new PIXI.Graphics();
                        ...
                        let atomCircle = new PIXI.Graphics();
        ...
        private PlayCircleAnimation(objForAnim: any): void {
        ...
        private DrawEnvironmentAndPlayer(loader: any, resources: any): void {
            ...
            for (let imageid of borderList) {
                ...
                let newSprite: CollisionShape = new CollisionShape(true, "rect", imageid[1], imageid[2], imageid[3], imageid[4], { isStatic: true, friction: 0, restitution: 1, collisionFilter: { category: 0x0001 } });
            ...
            for (let imageid of playerList) {
                let resource = resources[imageid[0]];
                let newSprite: CollisionShape = new CollisionShape(true, "circle", 200, 485, circleRad, circleRad, { isStatic: false, friction: 0, restitution: 1, inertia: Infinity, frictionAir: 0, frictionStatic: 0, collisionFilter: { category: 0x0002, mask: 0x0001 } });
            ...
            let rectColliderRight: CollisionShape = new CollisionShape(true, "rect", 375, 5, 50, 50, { isStatic: true, friction: 0, restitution: 1, collisionFilter: { category: 0x0001 }, angle: 45 });
            ...
            let rectColliderLeft: CollisionShape = new CollisionShape(true, "rect", -25, 5, 50, 50, { isStatic: true, friction: 0, restitution: 1, collisionFilter: { category: 0x0001 }, angle: 45 });
        ...
        private CalculateInOtherDirectionInitial(): number {
            let pos = { x: this.spriteArray["hand"].worldTransform.tx, y: this.spriteArray["hand"].worldTransform.ty };
            let firstDirection = { x: 1, y: 0 };
            let secondDirection = { x: pos.x - this.containerArray["EmitterContainer"].worldTransform.tx, y: pos.y - this.containerArray["EmitterContainer"].worldTransform.ty };
            this.movementVectorPlayer.x = secondDirection.x;
            this.movementVectorPlayer.y = secondDirection.y;
            let dotProduct = firstDirection.x * secondDirection.x + firstDirection.y * secondDirection.y;
            let determinant = firstDirection.x * secondDirection.y - firstDirection.y * secondDirection.x;
        ...
        private CalculateInOtherDirection(e?: any): number {
            let pos = e.data.global;
            let firstDirection = { x: 1, y: 0 };
            let secondDirection = { x: pos.x - this.containerArray["EmitterContainer"].worldTransform.tx, y: pos.y - this.containerArray["EmitterContainer"].worldTransform.ty };
            this.movementVectorPlayer.x = secondDirection.x;
        ...
        private SetPlayerMovementVector(e: any): void {
            if (this.pointerDown) {
                let ctr = 0;
                let _this = this;
                _this.currentlyPlayingPlayers = 0;
        ...
        private continueNormalSequence(): void {
            let _this = this;
            setTimeout(function () {
        ...
        protected CreatePlayersFromFakeOnes(): void {
            for (let fakePlayer of this.tweenedDownPlayer) {
                let timeLineTween = new TimelineMax({
        ...
        protected CreateNewPlayer(x: number, y: number, fakePlayer: any): void {
            // let toDestroy = this.fakePlayerArray[index][fakePlayer];
            // delete this.fakePlayerArray[index][fakePlayer];
            let toDestroy = fakePlayer;
            // delete fakePlayer;
            toDestroy.destroy();
            this.playerNum++;
            let newSprite: CollisionShape = new CollisionShape(true, "circle", x, y, circleRad, circleRad, { isStatic: false, friction: 0, restitution: 1, inertia: Infinity, frictionAir: 0, frictionStatic: 0, collisionFilter: { category: 0x0002, mask: 0x0001 } });
            newSprite.justRemoveBody();
            newSprite.beginFill(0xffffff, 1).drawCircle(0, 0, circleRad).endFill();
        ...
        protected MoveToFirstDrop(toMove: any): void {
        ...
        protected MoveDownAndDestroyAll(): void {
            ...
            for (let ctr = 0; ctr <= this.blockCouter; ctr++) {
                ...
                let timeOutValue = 0;
                if (Object.keys(this.destructibleCubeArray[ctr]).length > 0) {
